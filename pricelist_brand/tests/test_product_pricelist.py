...
class TestProductPricelist(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.list0 = cls.env["product.pricelist"].create({"name": "Base Pricelist"})
    ...
    def test_calculation_price_of_products_pricelist(self):
        ...
        price = self.pricelist._get_products_price(self.product, 1.0).get(
            self.product.id
        )
        self.assertEqual(
            float_compare(
                price,
                (self.product.lst_price - self.product.lst_price * (0.10)),
                precision_digits=2,
            ),
            0,
        )
        ...
        price_2 = self.pricelist._get_products_price(self.product_2, 1.0).get(
            self.product_2.id
        )
        self.assertEqual(
            float_compare(
                price_2,
                self.product_2.lst_price,
                precision_digits=2,
            ),
            0,
        )
