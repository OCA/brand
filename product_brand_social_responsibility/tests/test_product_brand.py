...
class TestBrandTags(CommonCase):
    ...
    def setUpClass(cls):
        ...
        cls.tag = cls.env["product.brand.tag.csr"].create(
            {
                "name": "Test Tag",
                "product_brand_ids": [
    ...
    def test_brand_rel(self):
        self.assertEqual(self.product_brand.csr_tag_ids, self.tag)
        self.assertEqual(self.product_brand2.csr_tag_ids, self.tag)
    ...
    def test_product_template_tag_uniq(self):
        with self.assertRaises(IntegrityError):
            with self.cr.savepoint():
                self.env["product.brand.tag.csr"].create({"name": "Test Tag"})

        self.env["product.brand.tag.csr"].create(
            {
                "name": "Test Tag",
